...
then   
  cd /tmp
  if [ \! -e /tmp/cmake-3.3.2-Linux-i386.tar.gz ]
  then
    wget -nv --no-check-certificate https://cmake.org/files/v3.3/cmake-3.3.2-Linux-i386.tar.gz
  fi
  tar xzf /tmp/cmake-3.3.2-Linux-i386.tar.gz
  cp cmake-3.3.2-Linux-i386/bin/* /usr/bin
  cp -r cmake-3.3.2-Linux-i386/share/* /usr/share
fi
...
then
  ...
  then
      wget --quiet -O jdk-7u79-linux-i586.tar.gz --no-cookies --no-check-certificate --header "Cookie: gpw_e24=http%3A%2F%2Fwww.oracle.com%2F; oraclelicense=accept-securebackup-cookie" "http://download.oracle.com/otn-pub/java/jdk/7u79-b15/jdk-7u79-linux-i586.tar.gz"
  fi
